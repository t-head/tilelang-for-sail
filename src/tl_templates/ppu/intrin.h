...
template <int thread_extent> TL_DEVICE bool tl_shuffle_elect() {
  ...
  if constexpr (thread_extent == 0) {
    ...
    //       0 to the whole warp to ensure the compiler knows it is
    //       warp-uniform, which helps the register allocator to assign the
    //       value to a uniform register. However, shfl.sync is dispatched to
    //       the LSU pipe, causing contention with other SMEM traffic, which
    //       can cause serious performance degradation in SMEM-intensive
    //       kernels. Here, we use elect.sync to inform the compiler that we
    //       are executing this with only one thread, so we do not need
    //       shfl.sync to make it warp-uniform.
