...
TL_DEVICE void cp_async_gs(void const *const smem_addr,
  ...
  if constexpr (N == 16) {
    asm volatile(
        ...
        "ppu.cp.async.cg.shared.global.L2::128B [%0], [%1], %2;"
#else
        "ppu.cp.async.cg.shared.global [%0], [%1], %2;"
#endif
        ::"r"(addr),
    ...
    asm volatile(
#if TL_ENABLE_L2_PREFETCH
        "ppu.cp.async.ca.shared.global.L2::128B [%0], [%1], %2;"
#else
        "ppu.cp.async.ca.shared.global [%0], [%1], %2;"
#endif
        ::"r"(addr),
        "l"((void const *)(global_ptr)), "n"(N));
  }
...
TL_DEVICE void cp_async_gs_conditional(void const *const smem_addr,
  ...
  if constexpr (N == 16) {
    asm volatile(
#if TL_ENABLE_L2_PREFETCH
        "ppu.cp.async.cg.shared.global.L2::128B [%0], [%1], %2, %3;"
#else
    ...
    asm volatile(
#if TL_ENABLE_L2_PREFETCH
        "ppu.cp.async.ca.shared.global.L2::128B [%0], [%1], %2, %3;"
#else
...
template <typename AccessType> struct global_load<AccessType, 4> {
  TL_DEVICE global_load(AccessType &D, void const *ptr, bool pred_guard) {
    ...
    asm volatile("{\n"
                 ...
                 "  ppu.mov.b32 %0, %3;\n"
#if TL_ENABLE_L2_PREFETCH
                 "  @p ppu.ld.global.L2::128B.u32 %0, [%1];\n"
#else
...
template <typename AccessType> struct global_load<AccessType, 8> {
  TL_DEVICE global_load(AccessType &D, void const *ptr, bool pred_guard) {
    ...
    asm volatile("{\n"
                 ...
                 "  ppu.mov.b32 %1, %5;\n"
#if TL_ENABLE_L2_PREFETCH
                 "  @p ppu.ld.global.L2::128B.v2.u32 {%0, %1}, [%2];\n"
#else
...
template <typename AccessType> struct global_load<AccessType, 16> {
  TL_DEVICE global_load(AccessType &D, void const *ptr, bool pred_guard) {
    ...
    asm volatile("{\n"
                 ...
                 "  ppu.mov.b32 %3, %9;\n"
#if TL_ENABLE_L2_PREFETCH
                 "  @p ppu.ld.global.L2::128B.v4.u32 {%0, %1, %2, %3}, [%4];\n"
#else
...
// Shared memory load/store intrinsics with explicit vector widths
