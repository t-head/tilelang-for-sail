...
namespace codegen {
namespace ppu {
using namespace tvm::tl::codegen::ppu;
using namespace ffi;
...
// PPU: renamed from Math (cloned from  codegen) to avoid confusing
// ""-branded identifiers inside the PPU backend; behavior unchanged.
struct PPUMath {
...
static std::string GetTileLangFP8Type(DataType type) {
  ...
  if (type.is_float8_e4m3() || type.is_float8_e4m3fn()) {
    ...
    LOG(FATAL) << "Unsupported FP8 type in HGGC codegen but got " << type;
  }
  return stream.str();
}
...
std::string GetTileLangFP6Type(DataType type) {
  ...
  if (type.code() == DataType::kFloat6_e2m3fn) {
    ...
    LOG(FATAL) << "Unsupported FP6 type in HGGC codegen";
  }
  stream << vec << suffix;
  return stream.str();
}
...
std::string GetTileLangFP4Type(DataType type) {
  ...
  if (type.is_float4_e2m1fn()) {
    ...
    LOG(FATAL) << "Unsupported FP4 type in HGGC codegen";
  }
...
void CodeGenTileLangPPU::PrintFuncPrefix(std::ostream &os) {
...
// PPU: detect whether the kernel uses Actlize MMA instructions (m16n16k*).
class ActlizeDetector : public tirx::StmtExprVisitor {
public:
  bool use_actlize_{false};

  void detect(const PrimFunc &f) {
    use_actlize_ = false;
    VisitStmt(f->body);
  }
...
private:
  void VisitExpr_(const CallNode *op) final {
    if (op->op.same_as(builtin::ptx_mma())) {
      ...
      ICHECK(n == 16) << "Actlize MMA requires n == 16, but got n = " << n;
      use_actlize_ = true;
    }
...
void CodeGenTileLangPPU::PrintExtraAttrs(const PrimFunc &f) {
...
std::string CodeGenTileLangPPU::Finish() {
  // HGGC 13's hggc.h declares CUtensorMap with `alignas(128)`, which MSVC
  ...
  // is 64. HGCC still feeds the kernel signature through MSVC's host
  ...
  if (enable_fp6_) {
    decl_stream << "// #include <hggc_fp6.h>  // not in PPU SDK\n";
  }
  if (enable_fp4_) {
    decl_stream << "#include <tl_templates/ppu/hggc_fp4.h>\n";
  }

  if (need_math_constants_h_) {
    decl_stream << "#include <hggc_math_constants.h>\n";
  }
  ...
  decl_stream << "#include <tl_templates/ppu/ldsm.h>\n";
  decl_stream << "#include <tl_templates/ppu/threadblock_swizzle.h>\n";
  decl_stream << "#include <tl_templates/ppu/debug.h>\n";
  decl_stream << "#ifdef ENABLE_BF16\n";
  decl_stream << "#include <tl_templates/ppu/hggc_bf16_fallbacks.h>\n";
  decl_stream << "#endif\n";
  decl_stream << "\n";
...
void CodeGenTileLangPPU::VisitStmt_(const tirx::ForNode *op) {
...
void CodeGenTileLangPPU::BindThreadIndex(const IterVar &iv) {
  ICHECK(!var_idmap_.count(iv->var.get()));
  var_idmap_[iv->var.get()] =
      CastFromTo(iv->thread_tag, DataType::UInt(32), iv->var.dtype());
}

void CodeGenTileLangPPU::PrintType(DataType t, std::ostream &os) { // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
  ...
  if (t.is_float()) {
    ...
    case 16:
      ...
      if (t.is_scalar()) {
        ...
        // Emit HGGC code to access fp16 vector elements.
        //
    ...
    case 32:
      if (lanes <= 4) {
        ...
        // Emit HGGC code to access fp32 vector elements for 4 < lanes <= 8.
        //
    ...
    // HGGC does not support bool vectors.
    ...
    case 1: {
      if (t.is_scalar()) {
        ...
        LOG(FATAL) << "Cannot convert type " << t << " to HGGC type!";
      }
    }
    case 4: {
      if (t.is_scalar()) {
    ...
    case 16: {
      if (t.is_scalar()) {
        ...
        // Emit HGGC code to access int16 vector elements.
        //
    ...
    case 32: {
      if (t.is_scalar()) {
        ...
        // Emit HGGC code to access int32 vector elements for 4 < lanes <= 8.
        //
...
void CodeGenTileLangPPU::PrintVecBinaryOp(const std::string &op, DataType t,
  ...
  // (__hadd2, __hsub2, etc.) are available on PPU0010+ (bf16) / (fp16).
  ...
  if (lanes >= 2 && lanes % 2 == 0) {
    ...
    if (CanEmitPackedX2Math(t)) {
      ...
      if (op == "+") {
        // Fuse packed mul+add here instead of relying on HGCC to recover
        // packed FMA from tl::mul2/tl::add2 (or the underlying __fmul2 /
        // __fadd2-style helpers). Once the pairwise ops are emitted as
        // separate calls, HGCC does not reliably contract them back to fma2.
      ...
      if (!tl_func.empty()) {
        ...
        //
        // Vector type → HGGC struct mapping:
        ...
        int ssa_scope = BeginScope();
        {
          ...
          if (is_bf16x2 || is_fp16x2) {
            std::string native_type = is_bf16x2 ? "__ppu_bfloat162" : "__half2";
...
void CodeGenTileLangPPU::PrintVecElemLoad(const std::string &vec, DataType t,
  ...
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    ...
    if (t.lanes() <= 8) {
      os << "((ppu_bfloat162*)(&(" << vec << "." << access[i / 2] << ")))->"
         << access[i % 2];
    } else {
      os << "(((ppu_bfloat162*)(&(" << vec << "." << access[i / 4] << "))) + "
         << (i / 2 % 2) << ")->" << access[i % 2];
    }
  } else if (t.is_float8()) {
    os << vec;
...
void CodeGenTileLangPPU::PrintVecElemStore(const std::string &vec, DataType t,
  ...
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    ...
    if (t.lanes() <= 8) {
      stream << "((ppu_bfloat162*)(&(" << vec << "." << access[i / 2] << ")))->"
             << access[i % 2] << " = " << value << ";\n";
    } else {
      stream << "(((ppu_bfloat162*)(&(" << vec << "." << access[i / 4]
             << "))) + " << (i / 2 % 2) << ")->" << access[i % 2] << " = "
             << value << ";\n";
    }
  } else if (t.is_float8()) {
...
void CodeGenTileLangPPU::PrintStorageSync(const CallNode *op) {
  auto args = op->args;
  ...
  if (sync == "warp") {
    ...
    LOG(FATAL) << "Cluster sync is not supported on PPU backend";
  } else if (sync == "global") {
...
bool IsHggcVectorizableFP8(DataType dtype) {
  return dtype.is_float8_e4m3() || dtype.is_float8_e4m3fn() ||
         dtype.is_float8_e5m2();
}

void CodeGenTileLangPPU::VisitExpr_(const CastNode *op, std::ostream &os) {
  ...
  if (from_ty.is_bfloat16() && target_ty.is_float() && target_ty.bits() == 32) {
    ...
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__bfloat1622float2", "__ppu_bfloat162", "float2", "",
                          true, false);
      return;
    }
  }
  ...
  if (from_ty.is_float() && from_ty.bits() == 32 && target_ty.is_bfloat16()) {
    ...
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__float22bfloat162_rn", "float2", "__ppu_bfloat162",
                          "", false, true);
      return;
    }
  }
  ...
  if (from_ty.is_float() && from_ty.bits() == 32 &&
    ...
    if (!cast_round.empty()) {
      LOG(FATAL) << "Unsupported rounding mode '" << cast_round
                 << "' for f32 -> FP8 cast on PPU.";
    }

    // Use __hg_cvt_float2_to_fp8x2 for vectorized conversion (float2 -> fp8x2)
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      std::string extra_args = ", __HG_SATFINITE, " + type_suffix;
      PrintVectorizedCast("__hg_cvt_float2_to_fp8x2", "float2",
                          "__hg_fp8x2_storage_t", extra_args, false, true);
      return;
    }
  }
  ...
  if (from_ty.is_float8_e8m0fnu() && target_ty.is_bfloat16()) {
    ...
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__tl_cvt_e8m0x2_to_bfloat162",
                          "__hg_fp8x2_storage_t", "__ppu_bfloat162", "", true,
                          false);
      return;
    }
  }
  ...
  if (from_ty.is_bfloat16() && target_ty.is_float8_e8m0fnu()) {
    ...
    // fp8_e8m0x2)
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__tl_cvt_bfloat162_to_e8m0x2", "__ppu_bfloat162",
                          "__hg_fp8x2_storage_t", "", false, true);
      return;
    }
  }
  ...
  if (from_ty.is_bfloat16() && target_ty.is_float4_e2m1fn()) {
    ...
    // fp4x2)
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__tl_cvt_bfloat162_to_fp4x2", "__ppu_bfloat162",
                          "uint8_t", "", false, true);
      return;
    }
  }
  ...
  if (from_ty.is_float4_e2m1fn() && target_ty.is_bfloat16()) {
    ...
    if (lanes == 2 || lanes == 4 || lanes == 8) {
      PrintVectorizedCast("__tl_cvt_fp4x2_to_bfloat162", "uint8_t",
                          "__ppu_bfloat162", "", true, false);
      return;
    }
  }

  // Reaching here with a non-empty `round` means this dtype pair has no
  // TIX `cvt.<round>` lowering; fail loudly instead of silently dropping it.
  if (!cast_round.empty()) {
...
void CodeGenTileLangPPU::VisitExpr_(const MinNode *op, std::ostream &os) {
  ...
  if (t.is_bfloat16() && t.is_scalar()) {
    os << "cutlass::bfloat16_t(__hmin("
       << "(" << PrintExpr(op->a) << ").to_ppu_bfloat16(), "
       << "(" << PrintExpr(op->b) << ").to_ppu_bfloat16()))";
    return;
  }
...
void CodeGenTileLangPPU::VisitExpr_(const MaxNode *op, std::ostream &os) {
  // TODO(wt): Consider vectorized reduction and impl for other dtypes
  DataType t = op->dtype;
...
/**
 * @brief Emit HGGC/TensorLib-specific code for a call expression.
 *
 * This visitor handles CallNode intrinsics and builtins that require emitting
 * HGGC/TL-specific code (inline TIX/ASM sequences, TensorLanguage runtime
 ...
 * commit/wait variants, tma_load/store and im2col variants, TIX
 * ldmatrix/stmatrix helpers, mbarrier APIs, cooperative grid sync, WMMA/legacy
 * MMA intrinsics (fill/load/store/mma/bmma/ptx_mma/ptx_mma_sp), low-level TIX
 ...
 */
void CodeGenTileLangPPU::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(tl::ppu_to_uniform_b32())) {
    ICHECK_EQ(op->args.size(), 1U);
    os << "__builtin_ppu_to_uniform_b32(" << PrintExpr(op->args[0]) << ")";
    return;
  }
  if (op->op.same_as(tl::max_nan()) || op->op.same_as(tl::min_nan())) {
    ...
    if (t.is_bfloat16() && t.is_scalar()) {
      os << "cutlass::bfloat16_t(" << f16_intrin << "("
         << "(" << PrintExpr(op->args[0]) << ").to_ppu_bfloat16(), "
         << "(" << PrintExpr(op->args[1]) << ").to_ppu_bfloat16()))";
      return;
    }
    if (t.is_float16() && t.is_scalar()) {
  ...
  if (op->op.same_as(builtin::ptx_cp_async())) {
    ...
    std::string func_name = "tl::tix_ldmatrix_x" + std::to_string(num);
    if (trans == 1) {
      func_name += "_trans";
      if (num == 4 && use_actlize_ldmatrix_) {
        func_name += "_ppu";
      }
    }
    print_extern_call_stmt(func_name, 2);
  } else if (op->op.same_as(tl::tix_ldmatrix_swzl())) {
    int trans = Downcast<IntImm>(op->args[0])->value;
    int num = Downcast<IntImm>(op->args[1])->value;
    std::string func_name = "tl::tix_ldmatrix_swzl_x" + std::to_string(num);
    if (trans == 1) {
      func_name += "_trans";
    }
    print_extern_call_stmt(func_name, 2);
  } else if (op->op.same_as(tl::ptx_stmatrix())) {
    LOG(FATAL) << "stmatrix is not supported on PPU backend";
  } else if (op->op.same_as(tl::fence_proxy_async()) ||
             op->op.same_as(tl::tma_store_arrive()) ||
             op->op.same_as(tl::tma_store_wait())) {
    LOG(FATAL) << "PPU only supports ppu0010/ppu0015; TMA async-proxy/store "
                  "completion operations require ppu0015+ TMA.";
  } else if (op->op.same_as(tl::warpgroup_arrive()) ||
             op->op.same_as(tl::warpgroup_commit_batch()) ||
             op->op.same_as(tl::warpgroup_wait()) ||
             op->op.same_as(tl::warpgroup_fence_operand()) ||
             op->op.same_as(tl::set_max_nreg())) {
    LOG(FATAL) << "PPU only supports ppu0010/ppu0015; warpgroup operations "
               << "(arrive/commit_batch/wait/fence_operand/reg_alloc) "
               << "require ppu0015+ Hopper.";
    ...
    LOG(FATAL) << "PPU only supports ppu0010/ppu0015; wait_wgmma requires "
               << "Hopper WGMMA.";
    ...
    this->PrintIndent();
    this->stream << "hggcTriggerProgrammaticLaunchCompletion();\n";
  } else if (op->op.same_as(tl::pdl_sync())) {
    this->PrintIndent();
    this->stream << "hggcGridDependencySynchronize();\n";
  } else if (op->op.same_as(tl::cluster_arrive_relaxed()) ||
             op->op.same_as(tl::cluster_arrive()) ||
             op->op.same_as(tl::cluster_wait()) ||
             op->op.same_as(tl::cluster_sync()) ||
             op->op.same_as(tl::block_rank_in_cluster()) ||
             op->op.same_as(tl::clc_try_cancel()) ||
             op->op.same_as(tl::clc_try_cancel_multicast()) ||
             op->op.same_as(tl::clc_is_canceled()) ||
             op->op.same_as(tl::clc_get_first_ctaid_x()) ||
             op->op.same_as(tl::clc_get_first_ctaid_y()) ||
             op->op.same_as(tl::clc_get_first_ctaid_z())) {
    LOG(FATAL) << "cluster operations are not supported on PPU backend";
    ...
    std::string BType = tl::codegen::ppu::tix::DTypeEnumToString(dtype_b_enum);
    std::string ARegType = tl::codegen::ppu::GetMMARegisterType(dtype_a_enum);
    if (ARegType == "float") {
      ARegType = "uint32_t";
    }
    std::string BRegType = tl::codegen::ppu::GetMMARegisterType(dtype_b_enum);
    if (BRegType == "float") {
      BRegType = "uint32_t";
    }
    ...
    replacer.register_rule("(CRegType)",
                           tl::codegen::ppu::GetMMARegisterType(dtype_c_enum));
    ...
    this->stream << replacer.rewrite(mma_call);
             ...
             op->op.same_as(tl::ptx_wgmma_sp_ss()) ||
             op->op.same_as(tl::ptx_wgmma_sp_rs())) {
    LOG(FATAL) << "PPU only supports ppu0010/ppu0015; WGMMA lowering requires "
               << "Hopper ppu0015+.";
  } else if (op->op.same_as(tl::ptx_tcgen05_mma_ss()) ||
             op->op.same_as(tl::ptx_tcgen05_mma_ts()) ||
             op->op.same_as(tl::ptx_tcgen05_mma_blockscaled_ss()) ||
             op->op.same_as(tl::ptx_tcgen05_cp_warpx4()) ||
             op->op.same_as(tl::ptx_tcgen05_sf_warp_transpose()) ||
             op->op.same_as(tl::tcgen05_ld()) ||
             op->op.same_as(tl::tcgen05_st()) ||
             op->op.same_as(tl::tcgen05_mma_arrive()) ||
             op->op.same_as(tl::tcgen05_before_thread_sync()) ||
             op->op.same_as(tl::tcgen05_after_thread_sync())) {
    LOG(FATAL) << "PPU only supports ppu0010/ppu0015; TCGEN05/TMEM lowering "
               << "requires Blackwell ppu0015+.";
    ...
    if (trans && op->dtype.bits() == 8) {
      ...
      if (trans == 1) {
        func_name += "_trans";
        if (num == 4 && type == ".b16" && use_actlize_ldmatrix_) {
          func_name += "_ppu";
        }
      }
      this->PrintIndent();
    ...
    LOG(FATAL) << "ptx_cp_async_bulk is not supported on PPU backend";
  } else if (op->op.same_as(builtin::ptx_commit_group())) {
    this->stream << "__asm__ __volatile__(\"ppu.cp.async.commit_group;\");\n\n";
  } else if (op->op.same_as(builtin::ptx_wait_group())) {
    int n = Downcast<IntImm>(op->args[0])->value;
    this->stream << "__asm__ __volatile__(\"ppu.cp.async.wait_group " << n
                 << ";\");\n\n";
  } else if (op->op.same_as(builtin::ptx_init_barrier_thread_count())) {
    ...
    this->stream << "\" ppu.cmpp.ne.b32 p, %2, 0;\\n\"\n";
    this->stream << "\" @!p ppu.mov.b32 %0, 0;\\n\"\n";
    this->stream << "\" @p ppu.ld.global.cr.f32 %0, [%1];}\\n\"\n";
    ...
    if (!src_dtype.is_float4_e2m1fn() && !tgt_dtype.is_float4_e2m1fn()) {
      ...
      // called for constants like -HGGCRT_INF_F), we need to create a temp
...
bool CodeGenTileLangPPU::HandleLateIntrinsicCall(const CallNode *op,
  ...
  if (op->op.same_as(tl::__exp())) {
    ...
    //
    // For float32x2 the HGGC type is float2 and C++ overload resolution
    // works directly.  For bfloat16x2 / float16x2 the HGGC type is uint1
    // (both map to the same 32-bit struct), so we must cast arguments to
    // the correct native type (__ppu_bfloat162 or __half2) and cast the
    ...
    if (op->op.same_as(tl::add2()) && op->args.size() == 2) {
      ...
      // same reason as PrintVecBinaryOp: HGCC will not reliably rewrite
    ...
    this->need_acrand_kernel_h_ = true;
    os << "acrand(&" << this->acrand_random_generator_state << ")";
    return true;
  } else if (op->op.same_as(tl::rng_rand_float())) {
    this->need_acrand_kernel_h_ = true;
    os << "acrand_" << op->args[0].as<StringImmNode>()->value;
    if (op->dtype.bits() == 64) {
      os << "_double";
    }
    os << "(&" << this->acrand_random_generator_state << ")";
    return true;
...
void CodeGenTileLangPPU::VisitStmt_(const AttrStmtNode *op) {
...
void CodeGenTileLangPPU::VisitStmt_(const AllocBufferNode *op) {
  ...
  if (scope.find("wmma.") == 0) {
    ...
    LOG(FATAL) << "PPU does not support WGMMA descriptor operations. "
...
void CodeGenTileLangPPU::VisitStmt_(const EvaluateNode *op) {
...
void CodeGenTileLangPPU::VisitExpr_(const RampNode *op, std::ostream &os) {
...
void CodeGenTileLangPPU::VisitStmt_(const BufferStoreNode *op) {
...
void CodeGenTileLangPPU::VisitExpr_(const ShuffleNode *op,
  ...
  if ((vec_is_bf16x2 || vec_is_fp16x2) && op->vectors.size() == 1 &&
    ...
    if (vec_is_bf16x2) {
      enable_bf16_ = true;
      os << "bfloat16_t(((ppu_bfloat162*)(&(" << vec << ")))->"
         << (lane == 0 ? "x" : "y") << ")";
    } else {
      enable_fp16_ = true;
  ...
  // hgcc rejects with "too many arguments in function call". Pack consecutive
...
void CodeGenTileLangPPU::VisitExpr_(const BroadcastNode *op,
  ...
  // `make_ulonglong4(v, v, v, v, v, v, v, v)` which hgcc rejects with
  ...
  // side-effecting call (acrand) executes once per uint32 lane, matching
  // the lane count.
  ...
  if (auto call = op->value.as<CallNode>()) {
    if (this->acrand_random_generator_state_type ==
        "acrandStatePhilox4_32_10_t") {
      if (call->op.same_as(tl::rng_rand()) && lanes == 4) {
        os << "acrand4(&" << this->acrand_random_generator_state << ")";
        return;
      }
      if (call->op.same_as(tl::rng_rand_float())) {
        ...
        if (bits == 32) {
          if (lanes == 4) {
            ...
            os << "acrand_normal2(&" << this->acrand_random_generator_state
               << ")";
            return;
          }
          ...
          if (lanes == 2) {
            os << "acrand_" << dist << "2_double(&"
               << this->acrand_random_generator_state << ")";
            return;
          }
        }
      }
    } else if (this->acrand_random_generator_state_type ==
...
inline void PrintConst(const FloatImmNode *op, std::ostream &os,
  ...
  case 32: {
    ...
    if (std::isinf(op->value)) {
      ...
      temp << ((op->dtype.bits() == 32) ? "HGGCRT_INF_F" : "HGGCRT_INF");
      p->need_math_constants_h_ = true;
    } else if (std::isnan(op->value)) {
      temp << ((op->dtype.bits() == 32) ? "HGGCRT_NAN_F" : "HGGCRT_NAN");
      p->need_math_constants_h_ = true;
    } else {
...
void CodeGenTileLangPPU::PrintWmmaScope(const std::string &scope, DataType t,
  ...
  if ((t.is_int() || t.is_uint()) && t.bits() < 8 && t.lanes() == 1) {
    ...
    if (t.is_int()) {
      if (t.bits() == 4) {
        type << "awmma::experimental::precision::s4";
      } else if (t.bits() == 1) {
        type << "awmma::experimental::precision::b1";
      } else {
        LOG(FATAL) << "Unhandled integer type for wmma fragment!";
      }
    } else if (t.is_uint()) {
      if (t.bits() == 4) {
        type << "awmma::experimental::precision::u4";
      } else {
        LOG(FATAL) << "Unhandled integer type for wmma fragment!";
      }
    }
...
void CodeGenTileLangPPU::PrintFunctionSignature(const String &function_name,
  ...
  // HGCC has issues with __restrict__ on kernel parameters when using PDL
  // (Programmatic Dependent Launch) synchronization. Suppress the annotation
  // when kHasGridSync is set.
  bool has_hggc_pdl_sync = func->HasNonzeroAttr(tl::attr::kHasGridSync);
  ...
  for (size_t i = 0; i < func->params.size(); ++i) {
    ...
    if (v.dtype().is_handle()) {
      ...
      if (!has_hggc_pdl_sync && no_alias && !non_restrict.count(v.get())) {
        PrintRestrict(v, os);
      }
    } else {
      CodeGenC::PrintType(GetType(v), os);
...
void CodeGenTileLangPPU::AddFunction(const GlobalVar &gvar, const PrimFunc &f) {
  ...
  if (code_block_source) {
    ...
    ICHECK(global_symbol) << "CodeGenTileLangPPU: Expect PrimFunc to have the "
    ...
    if (auto code_block_entry_name =
      ...
      ICHECK_EQ(static_cast<std::string>(global_symbol.value()),
          ...
          << "T.HGGCSourceCodeKernel expects the lowered device global_symbol "
             "to match entry_name";
    }
  ...
  // (Programmatic Dependent Launch) synchronization. Suppress the annotation
  // when kHasGridSync is set.
  bool has_hggc_pdl_sync = f->HasNonzeroAttr(tl::attr::kHasGridSync);
  std::unordered_set<const VarNode *> non_restrict;
  ...
  // Detect whether Actlize MMA (m16n16k*) is used, which requires the
  // PPU-specific ldmatrix layout.
  ActlizeDetector actlize_detector;
  actlize_detector.detect(f);
  use_actlize_ldmatrix_ = actlize_detector.use_actlize_;
