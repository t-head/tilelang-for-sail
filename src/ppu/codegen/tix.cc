...
// TIX related data structures and functions.
namespace tix {
...
/*!
 * \brief Create TIX data type from string.
 */
DataType DTypeFromString(const std::string str) {
  if (str == "int4" || str == ".s4") {
    ...
    LOG(FATAL) << "Unrecognized TIX data type " << str;
  }
}

std::string DTypeEnumToString(const tix::DataType &dtype) {
...
/*!
 * \brief Get the string representation of given TIX data type.
 */
...
/*!
 * \brief Get the number of bits of given TIX data type.
 */
...
void CheckMMAConfigValidity(int m, int n, int k, LayoutType layout_a,
  ...
  if (!(m == 8 && n == 8 && k == 4 && dtype_a == tix::DataType::kFloat16)) {
...
inline uint32_t GetNumMMAComputations(int m, int n, int k,
                                      tix::DataType dtype) {
  if (m == 8 && n == 8 && k == 4 && dtype == tix::DataType::kFloat16) {
...
GetMMAOperands(int m, int n, int k, tix::DataType dtype_a,
               tix::DataType dtype_b, tix::DataType dtype_c, bool sparse) {
  std::stringstream templates, inputs, outputs;
  const tix::FragAttrs frag_attr_a = tix::GetFragAttrs(dtype_a),
                       frag_attr_b = tix::GetFragAttrs(dtype_b),
                       frag_attr_c = tix::GetFragAttrs(dtype_c);
  constexpr uint32_t warp_size = 32;
  const uint32_t threads = warp_size / GetNumMMAComputations(m, n, k, dtype_a);
  const int num_operands_a = (m * k) * tix::DTypeBits(dtype_a) /
                             frag_attr_a.size / threads / (sparse ? 2 : 1),
            num_operands_b =
                (k * n) * tix::DTypeBits(dtype_b) / frag_attr_b.size / threads,
            num_operands_c =
                (m * n) * tix::DTypeBits(dtype_c) / frag_attr_c.size / threads;
...
PrintMMAAssembly(const std::string &shape, const std::string &A_layout,
  ...
  tix::DataType dtype_a = tix::DTypeFromString(A_dtype),
                dtype_b = tix::DTypeFromString(B_dtype),
                dtype_c = tix::DTypeFromString(C_dtype);
  if (dtype_a == tix::DataType::kFloat32) {
    dtype_a = tix::DataType::kTensorFloat32;
  }
  if (dtype_b == tix::DataType::kFloat32) {
    dtype_b = tix::DataType::kTensorFloat32;
  }
  tix::LayoutType layout_a = tix::LayoutTypeFromString(A_layout),
                  layout_b = tix::LayoutTypeFromString(B_layout);
  auto [m, n, k] = tix::ParseMMAShape(shape);
  ...
  std::string asm_code = R"(
  {
    ...
    __asm__ __volatile__(
        ...
        "ppu.tc01.ex.mma{.sparse}.sync.aligned{.shape}{.alayout}{.blayout}{.saturate}{.dtype}{.atype}{.btype}{.ctype}"
      #elif __HGGC_ARCH__ == 150
        "ppu.tc02.mma.sync.aligned{.shape}{.alayout}{.blayout}{.saturate}{.dtype}{.atype}{.btype}{.ctype}"
      #endif
        "{templates};\n"
        : {outputs}
        : {inputs});
  ...
  replacer.register_rule("{.atype}", tix::DTypeToString(dtype_a));
  replacer.register_rule("{.btype}", tix::DTypeToString(dtype_b));
  replacer.register_rule("{.ctype}", tix::DTypeToString(dtype_c));
  replacer.register_rule("{.dtype}", tix::DTypeToString(dtype_c));
...
std::string PrintLoadMatrixAssembly(bool trans, int num,
  ...
  tix::DataType data_type = tix::DTypeFromString(type);
  ICHECK(data_type == tix::DataType::kBit16)
  ...
  std::string asm_code = R"(
  {
    ...
    __asm__ __volatile__(
      #if __HGGC_ARCH__ == 100
        "ppu.tc01.ex.ldmatrix.sync.aligned{.shape}{.num}{.trans}{.ss}{.type}"
      #elif __HGGC_ARCH__ == 150
        "ppu.tc02.ldmatrix.sync.aligned{.shape}{.num}{.trans}{.ss}{.type}"
      #endif
        "{templates};\n"
        : {outputs}
        : "r"(addr)
  ...
  replacer.register_rule("{.type}", tix::DTypeToString(data_type));
...
std::string PrintCpAsyncAssembly(const std::string &shared_ptr,
  ...
  std::string asm_code = R"(
  {
    ...
    __asm__ __volatile__(
      #if __HGGC_ARCH__ == 100
        "ppu.cp.async.{cg_or_ca}.shared.global [%0], [%1], %2;"
      #elif __HGGC_ARCH__ == 150
        "ppu.cp.async.{cg_or_ca}.shared.global.LLC::128B [%0], [%1], %2;"
      #endif
...
std::string PrintPredicatedCpAsyncAssembly(
  ...
  std::string predicated_asm_code = R"(
  {
    ...
    __asm__ __volatile__(
      ...
      #if __HGGC_ARCH__ == 100
        " @p ppu.cp.async.{cg_or_ca}.shared.global [%1], [%2], %3;"
      #elif __HGGC_ARCH__ == 150
        " @p ppu.cp.async.{cg_or_ca}.shared.global.LLC::128B [%1], [%2], %3;"
      #endif
      "  @!p {store_shared};}"
  ...
  auto [store_shared, nopreg] = [](const std::string &bytes) {
    if (bytes == "16")
      return std::make_tuple("ppu.st.shared.v4.u32 [%1], {%4, %5, %6, %7}",
                             "\"r\"(0), \"r\"(0), \"r\"(0),\"r\"(0)");
    else if (bytes == "12")
      return std::make_tuple("ppu.st.shared.v3.u32 [%1], {%4, %5, %6}",
                             "\"r\"(0), \"r\"(0), \"r\"(0)");
    else if (bytes == "8")
      return std::make_tuple("ppu.st.shared.v2.u32 [%1], {%4, %5}",
                             "\"r\"(0), \"r\"(0)");
    else if (bytes == "4")
      return std::make_tuple("ppu.st.shared.u32 [%1], {%4}", "\"r\"(0)");
    else if (bytes == "2")
      return std::make_tuple("ppu.st.shared.u16 [%1], {%4}", "\"r\"(0)");
    else if (bytes == "1")
      return std::make_tuple("ppu.st.shared.u8 [%1], {%4}", "\"r\"(0)");
    else
...
std::string PrintCpAsyncBarrierAsm(const std::string &barrier) {
  std::string predicated_asm_code = R"(
  {
    ...
    __asm__ __volatile__(
      "ppu.cp.async.awbar.arrive.shared.b64 [%0];"
      :: "r" (barrier_addr_int)
    );
  }
)";
...
std::string PrintArriveBarrierAsm(const std::string &barrier) {
  std::string predicated_asm_code = R"(
  {
    unsigned int barrier_addr_int = cast_smem_ptr_to_int({barrier});
    __asm__ __volatile__(
      "{ .reg .b64 state; ppu.awbar.arrive.shared.b64 state, [%0]; }"
      :: "r"(barrier_addr_int)
    );
  }
)";
...
std::string PrintWaitBarrierAsm(const std::string &barrier) {
  std::string predicated_asm_code = R"(
  {
    ...
    __asm__ __volatile__(
      "{ .reg .pred P; WAIT: ppu.awbar.test_wait.parity.shared.b64 P, [%0], %1; @P ppu.bra DONE; ppu.bra WAIT; DONE: }"
...
std::string GetMMARegisterType(const tix::DataType &dtype) {
  switch (dtype) {
  case tix::DataType::kInt32:
    return "unsigned";
  case tix::DataType::kUInt32:
    return "unsigned";
  case tix::DataType::kFloat32:
    return "float";
  case tix::DataType::kFloat64:
    return "double";
  default:
    return "unsigned";
  }
