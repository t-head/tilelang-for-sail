...
/*!
 * \file tix.h
 * \brief Code generation with inlined TIX code.
 */
#ifndef TVM_TL_TARGET_SOURCE_TIX_PPU_H_
#define TVM_TL_TARGET_SOURCE_TIX_PPU_H_
...
namespace tvm::tl {
namespace codegen {
// namespace so the two copies can coexist in one libtilelang without
namespace ppu {
...
/*!
 * \brief Print tix data type from string.
 */
DataType DTypeFromString(const std::string str);

/*!
 * \brief Print tix data type from enum.
 */
...
/*!
 * \brief Print tix cp.async assembly string given parameters.
...
/*!
 * \brief Print tix cp.async assembly string given parameters (no offset
 * version).
 * \param shared_ptr: The pointer to the destination shared memory.
 * \param global_ptr: The pointer to the global memory.
 * \param bytes: The number of bytes to copy, valid values are 4, 8, and 16.
...
/*!
 * \brief Print predicated tix cp.async assembly string given parameters.
 * \param shared_ptr: The pointer to the destination shared memory.
 * \param shared_elem_offset: The offset into the shared memory (empty for no
 * offset).
 * \param global_ptr: The pointer to the global memory.
...
/*!
 * \brief Print predicated tix cp.async assembly string given parameters (no
 * offset version).
 * \param shared_ptr: The pointer to the destination shared memory.
 * \param global_ptr: The pointer to the global memory.
 * \param bytes: The number of bytes to copy, valid values are 4, 8, and 16.
...
/*!
 * \brief Print tix async copy barrier using cp.async.mbarrier.arrive
 * \param barrier: The name of the barrier in shared memory.
 */
std::string PrintCpAsyncBarrierAsm(const std::string &barrier);

/*!
 * \brief Print tix barrier initialization of thread count using mbarrier.init
 * \param barrier: The name of the barrier in shared memory.
...
/*!
 * \brief Print tix barrier arrival using mbarrier.arrive
 * \param barrier: The name of the barrier in shared memory.
 */
std::string PrintArriveBarrierAsm(const std::string &barrier);

/*!
 * \brief Print tix barrier wait using mbarrier.try_wait
 * \param barrier: The name of the barrier in shared memory.
 */
...
#endif // TVM_TL_TARGET_SOURCE_TIX_PPU_H_
