/*!
 * \file lower_pdl.cc
 * \brief Mark Device PrimFunc with attributes if PDL functions are called
 */
...
// HGCC has issues with __ldg when using PDL (Programmatic Dependent Launch)
...
class MarkPpuSyncCalls : public StmtExprMutator {
public:
  static PrimFunc Substitute(PrimFunc f, bool support_pdl) {
    MarkPpuSyncCalls mutator;
    PrimFunc new_f = f;
...
private:
  ...
  MarkPpuSyncCalls() = default;
};

using namespace tirx::transform;

static tvm::transform::Pass MarkPpuSyncCallsPass(bool support_pdl) {
  auto pass_func = [=](PrimFunc f, const IRModule &m, const PassContext &ctx) {
    return MarkPpuSyncCalls::Substitute(f, support_pdl);
  };

  return CreatePrimFuncPass(pass_func, 0, "tl.MarkPpuSyncCalls", {});
}

TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = tvm::ffi::reflection;
  refl::GlobalDef().def("tl.ppu.transform.MarkPpuSyncCalls",
                        MarkPpuSyncCallsPass);
}
