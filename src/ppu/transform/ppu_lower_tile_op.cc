...
private:
  ...
  PrimExpr ApplyWarpBinding(PrimExpr offset) {
    if (!use_warp_binding_ || !thread_var_.defined()) {
      return offset;
    }
    Var tx = thread_var_->var;
    PrimExpr warp_base = floordiv(tx, kWarpSize) * kWarpSize;
    Map<Var, PrimExpr> subst_map;
    subst_map.Set(tx, warp_base);
    return analyzer_->Simplify(tirx::Substitute(offset, subst_map));
  }
  ...
  HandleAccessPtrAndOffset(const PrimExpr &access_ptr,
    ...
    if (access_ptr_call->op.same_as(builtin::tvm_access_ptr())) {
      ...
      // In AIU context, swizzle is encoded in instruction operands rather
      // than in memory indices, so we only update buffer data without
      // recomputing indices.
      if (in_aiu_context_) {
      ...
      new_offset = ApplyWarpBinding(new_offset);
  ...
  Stmt VisitStmt_(const ForNode *op) final {
    ...
    PostOrderVisit(for_node->body, [&](const ObjectRef &obj) {
      if (const auto *cast = obj.as<CastNode>()) {
        ...
        if (IsPpuVectorizableCast(from_ty, target_ty)) {
  ...
  // Flag to indicate we are inside an AIU context. When true,
  // HandleAccessPtrAndOffset only updates buffer data without recomputing
  // indices, since swizzle is encoded in instruction operands rather than in
  // memory indices.
  bool in_aiu_context_{false};
  bool use_warp_binding_{false};
...
tvm::transform::Pass PpuLowerTileOp() {
  auto pass_func = [=](PrimFunc f, const IRModule &m, const PassContext &ctx) {
    return LowerTileOpPass::Substitute(std::move(f));
  };
  return CreatePrimFuncPass(pass_func, 0, "tl.ppu.LowerTileOp", {});
}

TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = reflection;
  refl::GlobalDef().def("tl.ppu.transform.LowerTileOp", PpuLowerTileOp);
}
