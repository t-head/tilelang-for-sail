/*!
 * \brief PPU-specific layout inference pass with trans_buffer support.
 * \file ppu_layout_inference.cc
 *
 * This is the PPU-specific version of LayoutInference that handles
 * GEMM RS SRCA layout conflicts via the trans_buffer mechanism.
 * When a shared buffer is used by multiple Gemm ops requiring different
 * layouts (e.g., swizzle vs fragment), this pass creates alternative
 * buffers and rewires the IR accordingly.
 *
 * For non-PPU targets, use the common LayoutInference from
 * src/transform/layout_inference.cc which does not support trans_buffer.
 */
...
static std::string NextUniqueId() {
...
struct LayoutInferenceResult {
  ...
  Map<ObjectRef, Map<Buffer, Buffer>> trans_buffer;
};
    ...
    for (const auto &[buffer, layout] : updates) {
      ...
      if (layout_map.count(buffer)) {
        ...
        if (!layout->IsEqual(layout_map[buffer].get())) {
          ...
          const auto *gemm = next.as<GemmNode>();
          if (!gemm) {
          ...
          ObjectRef op = infer_list_stmt_[cur_infer_id];
          bool found = false;
          for (const auto &[map_buffer, map_layout] : layout_map) {
            std::string name = map_buffer->name;
            if (layout->IsEqual(map_layout.get()) &&
                name.rfind(std::string(buffer->name), 0) == 0) {
              Map<Buffer, Buffer> trans;
              trans.Set(buffer, map_buffer);
              trans_buffer_.Set(op, trans);
              found = true;
              break;
            }
          }
          if (!found) {
            Buffer new_buffer = CreateNewBuffer(buffer, layout, NextUniqueId());
            layout_map.Set(new_buffer, layout);
            Map<Buffer, Buffer> trans;
            trans.Set(buffer, new_buffer);
            trans_buffer_.Set(op, trans);
          }
        }
...
private:
  ...
  Buffer CreateNewBuffer(const Buffer &orig, const Layout &new_layout,
                         const std::string &id) {
    String new_name = orig->name + id;
    Type ptr_type = orig->data->type_annotation.defined()
                        ? orig->data->type_annotation
                        : PointerType(PrimType(orig->dtype), "global");
    Var new_data(new_name, ptr_type);
    ObjectPtr<BufferNode> node = make_object<BufferNode>(*orig.get());
    node->data = new_data;
    node->shape = new_layout->InputShape();
    node->name = new_name;
    return Buffer(node);
  }

  Map<ObjectRef, Map<Buffer, Buffer>> trans_buffer_;
};
...
private:
  ...
  Stmt VisitStmt_(const SBlockNode *op) final {
    ...
    std::unordered_set<Buffer, ObjectPtrHash, ObjectPtrEqual> added;
    for (const Buffer &buffer : block->alloc_buffers) {
      ...
      for (const auto &[_, trans] : result_.trans_buffer) {
        if (trans.count(buffer)) {
          Buffer transformed = trans[buffer];
          if (added.insert(transformed).second) {
            new_alloc_buffers.push_back(transformed);
          }
        }
      }
    }
  ...
  FindTransformedBuffer(const Map<Buffer, Buffer> &trans,
    ...
    for (const auto &[original, transformed] : trans) {
      if (original.same_as(buffer) || original->data.same_as(buffer->data) ||
  ...
  Stmt VisitStmt_(const EvaluateNode *op) final {
    ...
    if (call->op.same_as(Gemm::Get()) && result_.trans_buffer.count(call)) {
      ...
      Array<PrimExpr> region_args = b_region->args;
      region_args.Set(0, BufferLoad(transformed.value(), b_load->indices));
      Array<PrimExpr> call_args = call->args;
      call_args.Set(1, Call(b_region->dtype, b_region->op, region_args,
                            b_region->annotations, b_region->span));
      return Evaluate(Call(call->dtype, call->op, call_args, call->annotations,
                           call->span));
    }
    if (call->op.same_as(Copy::Get())) {
      Call dst_region = Downcast<Call>(call->args[1]);
      BufferLoad dst_load = Downcast<BufferLoad>(dst_region->args[0]);
      Array<Stmt> copies;
      std::unordered_set<Buffer, ObjectPtrHash, ObjectPtrEqual> copied;
      for (const auto &[_, trans] : result_.trans_buffer) {
        Optional<Buffer> transformed =
            FindTransformedBuffer(trans, dst_load->buffer);
        if (!transformed.defined() ||
            !copied.insert(transformed.value()).second) {
          continue;
        }
        Array<PrimExpr> region_args = dst_region->args;
        region_args.Set(0, BufferLoad(transformed.value(), dst_load->indices));
        Array<PrimExpr> call_args = call->args;
        call_args.Set(1, Call(dst_region->dtype, dst_region->op, region_args,
                              dst_region->annotations, dst_region->span));
        copies.push_back(Evaluate(Call(call->dtype, call->op, call_args,
                                       call->annotations, call->span)));
      }
      if (!copies.empty()) {
        copies.push_back(GetRef<Stmt>(op));
        return SeqStmt(copies);
      }
    }
    return IRMutatorWithAnalyzer::VisitStmt_(op);
  }
...
tvm::transform::Pass PpuLayoutInference() {
  ...
  return CreatePrimFuncPass(pass_func, 0, "tl.ppu.LayoutInference", {});
}

TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = reflection;
  refl::GlobalDef().def("tl.ppu.transform.LayoutInference", PpuLayoutInference);
}
