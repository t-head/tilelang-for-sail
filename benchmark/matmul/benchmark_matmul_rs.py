...
def ref_program(A, B):
    ...
    matmul_rs computes C = A @ B with trans_A=True (K-major A) and trans_B=False (K-major B),
    which means A has shape (K, M) and B has shape (K, N), result C is (M, N).
    Internally A is loaded into register (fragment) before MMA.

    Parameters
    ----------
    A : numpy.ndarray
        The matrix with shape (K, M).
    B : numpy.ndarray
    ...
    -------
    np.ndarray
        The result of A.T @ B, shape (M, N).
    """
    return A.T @ B


def get_configs(M, N, K, with_roller, in_dtype=T.float16, out_dtype=T.float16, accum_dtype=T.float32, **kwargs):
    """
...
def matmul(
    ...
    """
    Create an autotuned register-source matrix multiplication kernel for matrices of shape:
      - A: (K, M)   -- trans_A=True, K-major
      - B: (K, N)   -- trans_B=False, K-major
      - C: (M, N)

    A is loaded via shared memory into register (fragment) before MMA,
    which is called "register source" for the A operand.
    B remains in shared memory ("shared source").
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Autotuned Register-Source MatMul Benchmark")
    ...
    # Run autotuning
    best_result = matmul(M, N, K, with_roller, in_dtype, out_dtype, accum_dtype)
