"""DeepSeek MLA Decode (Paged KV cache) benchmark with autotune.

Adapted from: examples/deepseek_mla/example_mla_decode_paged.py
Tunable parameters: block_N, block_H, num_stages, threads

Difference vs benchmark_mla.py:
    - KV cache is stored in a flat page pool and addressed via a block_table
      (PagedAttention style), with per-batch variable sequence lengths
      (cache_seqlens) and causal masking.
"""
...
def mla_decode_paged(
    ...
    """Paged MLA decode kernel with online softmax.
    ...
    def main_split(
        ...
        with T.Kernel(batch, h_q // min(block_H, kv_group_num), num_split, threads=threads) as (bx, by, bz):
    ...
    def main_no_split(
        ...
        with T.Kernel(batch, h_q // min(block_H, kv_group_num), threads=threads) as (bx, by):
            Q_shared = T.alloc_shared([block_H, dv], dtype)
            S_shared = T.alloc_shared([block_H, block_N], dtype)
            Q_pe_shared = T.alloc_shared([block_H, dpe], dtype)
            KV_shared = T.alloc_shared([block_N, dv], dtype)
...
def scaled_dot_product_attention(query, key, value, h_q, h_kv, is_causal=False):
    """Reference SDPA (from example_mla_decode_paged.py)."""
    query = query.float()
    key = key.float()
    value = value.float()
...
def run_profile(batch, h_q, h_kv, cache_seqlen, d, dv, block_N, block_H, num_stages, threads):
    ...
    glse = torch.empty(batch, h_q, num_split, dtype=dtype, device=device)
    out_partial = torch.empty(batch, h_q, num_split, dv, dtype=dtype, device=device)
...
def run_profile_ref(batch, h_q, h_kv, cache_seqlen, d, dv):
...
def main(batch=128, h_q=128, h_kv=1, cache_seqlen=8192, d=576, dv=512):
    ...
    # Split into nope / pe parts (kernel takes them separately)
    q_nope, q_pe = q[..., :dv].contiguous(), q[..., dv:].contiguous()
    blocked_k_nope = blocked_k[..., :dv].contiguous()
    blocked_k_pe = blocked_k[..., dv:].contiguous()
    ...
    # Reference: per-batch paged SDPA (from example run_torch_mla)
    def ref_paged_mla():
        out = torch.empty(batch, s_q, h_q, dv, dtype=torch.float32, device=device)
        for i in range(batch):
            begin = i * max_seqlen_pad
            end = begin + cache_seqlens[i]
    ...
    ref_latency = bench_ref(ref_paged_mla)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Paged MLA Decode Autotune Benchmark")
    parser.add_argument("--batch", type=int, default=128)
    parser.add_argument("--h_q", type=int, default=128)
