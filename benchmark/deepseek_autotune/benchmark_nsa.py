"""DeepSeek NSA (Native Sparse Attention) fwd benchmark with autotune.

Adapted from: examples/deepseek_nsa/example_tilelang_nsa_fwd.py
...
def nsa_fwd(
    ...
    """NSA forward kernel with online softmax.
...
def run_profile_ref(batch, heads, seq_len, dim, selected_blocks, block_size, is_causal):
    """Run the reference once for ncu/acu profiling."""
    import torch

    scale = 0.1
    total_kv = selected_blocks * block_size
    Q = torch.randn(batch, heads, seq_len, dim, dtype=torch.float16, device="cuda")
    K = torch.randn(batch, heads, seq_len, total_kv, dim, dtype=torch.float16, device="cuda")
    V = torch.randn(batch, heads, seq_len, total_kv, dim, dtype=torch.float16, device="cuda")

    scores = torch.einsum("bhsd,bhstd->bhst", Q.float(), K.float()) * scale
    attn = scores.softmax(dim=-1)
    torch.einsum("bhst,bhstd->bhsd", attn, V.float()).half()
...
def main(batch=2, heads=16, seq_len=64, dim=32, selected_blocks=1, block_size=32, is_causal=True):
    ...
    # Reference: naive attention over selected blocks using torch
    total_kv = selected_blocks * block_size
    Q = torch.randn(batch, heads, seq_len, dim, dtype=torch.float16, device="cuda")
    K = torch.randn(batch, heads, seq_len, total_kv, dim, dtype=torch.float16, device="cuda")
    V = torch.randn(batch, heads, seq_len, total_kv, dim, dtype=torch.float16, device="cuda")
    ...
    def ref_nsa():
        scores = torch.einsum("bhsd,bhstd->bhst", Q.float(), K.float()) * scale
        attn = scores.softmax(dim=-1)
        return torch.einsum("bhst,bhstd->bhsd", attn, V.float()).half()

    ref_latency = bench_ref(ref_nsa)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NSA Fwd Autotune Benchmark")
