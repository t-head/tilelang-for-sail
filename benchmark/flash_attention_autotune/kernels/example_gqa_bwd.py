...
def flashattn_bwd_atomic_add(batch, heads, seq_len, dim_qk, dim_v, is_causal, block_M=64, block_N=64, threads=256, num_stages=2, groups=1):
...
def flashattn_bwd_split(batch, heads, seq_len, dim_qk, dim_v, is_causal, block_M=64, block_N=64, threads=256, num_stages=2, groups=1):
    sm_scale = (1.0 / dim_qk) ** 0.5
    scale = (1.0 / dim_qk) ** 0.5 * 1.44269504  # log2(e)
    head_kv = heads // groups
    q_shape = [batch, seq_len, heads, dim_qk]
...
def main(
    ...
    if not tune:
        ...
        # 定义当前的注意力配置
        _attention.current_config = {"block_M": block_M, "block_N": block_N, "num_stages": num_stages, "threads": threads}
        O = attention(Q, K, V, causal, groups)
        O.backward(dO, retain_graph=True)
    else:
        kernel = flashattn_bwd_atomic_add(BATCH, H, N_CTX, D_HEAD_QK, D_HEAD_V, causal, groups=groups)
        best_config = kernel.config
        best_latency = kernel.latency

        # run only once for grad accuracy
        _attention.current_config = best_config
        O_best = attention(Q, K, V, causal, groups)
        Q.grad = None
        K.grad = None
        V.grad = None
        O_best.backward(dO)
        ...
        return O_best, best_latency, total_flops / best_latency * 1e-9, best_config
...
if __name__ == "__main__":
    ...
    parser.add_argument("--block_M", type=int, default=None, help="Block size for dimension M (rows). Common: 16, 32, 64, 128")
    parser.add_argument("--block_N", type=int, default=None, help="Block size for dimension N (cols). Common: 16, 32, 64, 128")
    parser.add_argument("--num_stages", type=int, default=2, help="Number of pipeline stages for latency hiding. Common: 2, 3, 4")
    parser.add_argument(
        "--threads", type=int, default=256, help="Number of threads per block. Must be multiple of 32. Common: 128, 256, 512"
    )
