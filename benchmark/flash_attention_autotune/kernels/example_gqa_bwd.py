...
def main(BATCH: int = 1,
    ...
    if (not tune):
        ...
        head_kv = H // groups
        K = torch.empty(BATCH, N_CTX, head_kv, D_HEAD_QK, dtype=torch.half,
                        device="cuda").normal_().requires_grad_()
        V = torch.empty(BATCH, N_CTX, head_kv, D_HEAD_V, dtype=torch.half,
                        device="cuda").normal_().requires_grad_()
        dO = torch.empty(BATCH, N_CTX, H, D_HEAD_V, dtype=torch.half,
                        device="cuda").normal_().requires_grad_()
        # 定义当前的注意力配置
        _attention.current_config = {
            "block_M": block_M,
            "block_N": block_N,
...
if __name__ == "__main__":
    ...
    main(args.batch, args.h, args.n_ctx, args.d_head_qk, args.d_head_v, args.groups, args.causal, args.tune,
        block_M=args.block_M, block_N=args.block_N, num_stages=args.num_stages, threads=args.threads)