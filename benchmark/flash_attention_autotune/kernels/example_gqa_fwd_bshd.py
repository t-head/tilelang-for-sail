...
def main(batch: int = 1,
    ...
    if (not tune):
        ...
        head_kv = heads // groups
        K = (
            torch.empty(batch, seq_len, head_kv, dim, dtype=torch.half,
                        device="cuda").normal_().requires_grad_())
        V = (
            torch.empty(batch, seq_len, head_kv, dim, dtype=torch.half,
                        device="cuda").normal_().requires_grad_())
        ...
        best_kernel = flashattn(batch, heads, seq_len, dim, is_causal, groups,
...
if __name__ == "__main__":
    ...
    main(args.batch, args.heads, args.seq_len, args.dim, args.causal, args.groups, False,
